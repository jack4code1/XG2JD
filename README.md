...
### 压测结果
```
三轮并发矩阵（10/50/100/200 并发，共 4,800 请求）：成功率 100%，0 重复订单
200 并发持续实验（2,000 请求）：419.44 req/s，P50 462.84ms，P99 491.31ms
库存耗尽实验（库存 100 / 请求 300）：成功 100，库存 0，无超卖
同一用户竞争（50 并发 / 请求 100）：成功 1，库存仅扣减 1
```

以上为 2026-08-19 在 i7-12650H、15.8GB 内存、本机 Docker 中间件、单应用实例上的实测结果。详细方法、三轮数据、实验边界和复现命令见：[并发实验报告](docs/BENCHMARK_REPORT.md)。
