...
export default function Shop() {
  ...
  return (
    <div>
      <Link to="/" className="btn btn-ghost btn-sm mb-4">← 返回商城</Link>
      <div className="card bg-base-100 shadow-lg mb-6">
        <div className="card-body">
          <h1 className="text-2xl font-bold">{merchant.shopName}</h1>
          <p className="text-base-content/60">{merchant.shopDesc || merchant.category}</p>
        </div>
      </div>
          ...
          return (
            <div key={c.id} className="card card-side bg-base-100 shadow-lg">
              ...
              <div className="card-actions items-center p-4">
                <button onClick={() => seckill(c.id)} disabled={!active}
                  className={`btn ${active ? 'btn-primary' : 'btn-disabled'}`}>
                  {active ? '⚡ 抢券' : '已结束'}
                </button>
              </div>
            </div>
      ...
      {result && (
        <div className="modal modal-open">
          <div className="modal-box text-center">
            ...
            <h3 className={`text-xl font-bold ${result.success ? 'text-success' : 'text-error'}`}>
            ...
            <div className="modal-action justify-center">
              <button onClick={() => setResult(null)} className="btn btn-primary">确定</button>
            </div>
          </div>
        </div>
      )}
