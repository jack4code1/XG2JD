...
export default function Shop({ shopId, onBack, onOrders }) {
  ...
  return <>
    ...
    <div className="coupon-list">{(m.coupons || []).map(c => { const now = Date.now(); const start = new Date(c.startTime).getTime(); const end = new Date(c.endTime).getTime(); const remain = c.remainStock || 0; const total = c.totalStock || 100; const pct = Math.max(0, remain / total * 100); const notStarted = now < start; const ended = now >= end || c.status === 2; const active = !notStarted && !ended && c.status === 1; const timeText = notStarted ? `将于 ${formatTime(c.startTime)} 开始` : ended ? `已于 ${formatTime(c.endTime)} 结束` : `进行中 · 截止 ${formatTime(c.endTime)}`; const buttonText = notStarted ? '未开始' : ended ? '已结束' : remain > 0 ? '立即领' : '已领光'; return <div className="coupon" key={c.id}><div className="coupon-value">抵扣<br /><small>¥{Number(c.discountAmount || 0).toFixed(0)}</small></div><div className="coupon-info"><strong>{c.couponName}</strong><small>剩余 {remain} / {total}　·　每人限领 {c.perUserMax || 1} 张</small><span className="coupon-time">{timeText}</span><div className="stock"><i style={{ width: (100 - pct) + '%' }} /></div></div><button className="primary-btn" onClick={() => go(c.id)} disabled={!active || remain < 1}>{buttonText}</button></div> })}</div>
