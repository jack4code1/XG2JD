    ...
    <div>
      <div className="hero bg-base-200 rounded-box mb-8 py-12">
        <div className="hero-content text-center">
          <div>
            <h1 className="text-4xl font-bold gradient-text">🔥 限时秒杀</h1>
            <p className="py-4 text-base-content/60">精选商家 · 超值优惠券 · 手慢无</p>
          </div>
        </div>
      </div>
        ...
        {merchants.map(m => (
          <Link to={`/shop/${m.id}`} key={m.id}
            ...
            <div className="card-body items-center text-center p-6">
              ...
              <p className="text-sm text-base-content/60">{m.couponCount} 张优惠券</p>
            </div>
          </Link>
        ))}
        {merchants.length === 0 && (
          <div className="col-span-full text-center py-12 text-base-content/40">
            <p className="text-lg">暂无商家入驻</p>
          </div>
        )}
      </div>
    </div>
