...
export default function Orders() {
  ...
  return (
    <div>
      ...
      {orders.length===0 ? (
        <div className="text-center py-12 text-base-content/40"><p>暂无订单</p></div>
      ) : (
        <div className="overflow-x-auto">
          <table className="table">
            <thead><tr><th>订单号</th><th>商家</th><th>优惠券</th><th>状态</th><th>操作</th></tr></thead>
            <tbody>
              {orders.map(o=>(
                <tr key={o.orderNo}>
                  <td className="font-mono text-xs">{o.orderNo?.slice(0,14)}...</td>
                  <td>{o.shopName||'-'}</td><td>{o.couponName||'-'}</td>
                  ...
                  <td>{o.status==='CREATED'&&<div className="flex gap-1"><button onClick={()=>pay(o.orderNo)} className="btn btn-xs btn-primary">支付</button><button onClick={()=>cancel(o.orderNo)} className="btn btn-xs btn-ghost">取消</button></div>}</td>
                </tr>
              ))}
            </tbody>
          </table>
