...
export default function AiTab() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPlan('AI服务暂不可用，请确保已设置 DEEPSEEK_API_KEY')
    }
    setLoading(false)
  }
      ...
      <GlassCard>
        <div className="text-center py-4">
          ...
          <p className="text-sm text-gray-500">
            4个AI Agent并行协作：📊数据分析 · 🛡️风控评估 · ✍️内容生成 · 📈策略推荐
          </p>
        </div>
      </GlassCard>
      ...
      <GlassCard>
        <form onSubmit={handleSubmit} className="flex gap-3">
          <input
            ...
            placeholder="描述你的活动需求，例如：帮我策划一个双11秒杀..."
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {EXAMPLES.map((ex, i) => (
            <button key={i} onClick={() => { setQuery(ex); handleSubmit() }}
              className="text-xs glass glass-hover px-3 py-1.5 rounded-lg text-gray-400 hover:text-white transition-colors">
              {ex}
            </button>
          ))}
        </div>
            ...
            <div className="flex gap-1">
              {[0,1,2,3].map(i => (
                <motion.div key={i}
                  ...
                  style={{ background: ['#6366f1','#10b981','#f59e0b','#8b5cf6'][i] }}
                />
              ))}
            </div>
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { navigator.clipboard.writeText(plan) }}
                className="text-xs glass glass-hover px-3 py-1 rounded-lg text-gray-400 hover:text-white">
                📋 复制
              </button>
            </div>
            <div className="text-sm whitespace-pre-wrap leading-relaxed text-gray-300 font-mono text-xs">
              {plan}
            </div>
          </GlassCard>
        </motion.div>
