...
export default function Admin() {
  ...
  if (!isLogin) { nav('/login'); return null }
  ...
  const create = async (e) => {
    ...
    } catch(e) { setMsg('❌ 创建失败: ' + (e.response?.data?.message || e.message)) }
  }
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="card-body">
            ...
            <form onSubmit={create}>
              <input value={form.couponName} onChange={e=>setForm({...form,couponName:e.target.value})}
                placeholder="优惠券名称" required className="input input-bordered w-full mb-3" />
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div><label className="label text-xs">总库存</label>
                ...
                <div><label className="label text-xs">每人限购</label>
                  <input type="number" value={form.perUserMax} onChange={e=>setForm({...form,perUserMax:e.target.value})}
                    className="input input-bordered w-full" /></div>
                <div><label className="label text-xs">时长(小时)</label>
        ...
        <div className="card bg-base-100 shadow-lg">
          <div className="card-body">
            <h3 className="card-title">⚡ 故障演练</h3>
            <div className="grid grid-cols-2 gap-2">
              {['redis','mq','db','network'].map(s => (
                <button key={s} onClick={async () => {
                  await client.post(`/drill/start?scenario=${s}`)
                  await new Promise(r=>setTimeout(r,3000))
                  const { data } = await client.post('/drill/recover')
                  alert(`${s} 恢复完成: ${data.durationMs}ms`)
                }} className="btn btn-outline btn-sm">
                  {s==='redis'?'🔴 Redis':s==='mq'?'🟡 MQ':s==='db'?'🟠 DB':'🔵 网络'}
                </button>
              ))}
            </div>
          </div>
