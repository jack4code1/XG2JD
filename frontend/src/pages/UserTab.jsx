...
export default function UserTab() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      if (data?.length) {
        ...
        // Use username hash as uid fallback for demo
        try {
          const { data: od } = await client.get(`/order/user/${cid}`)
  ...
  const handleRegister = async (e) => {
    e.preventDefault()
    try { const r = await register(username, password); alert(r.message || '注册成功') }
    catch { alert('注册失败') }
  }
  ...
  const handleSeckill = async (couponId) => {
    ...
    try {
      ...
      setResult({ success: false, message: '系统繁忙', elapsed: Math.round(performance.now() - t0) })
    }
    setLoading(false)
  }
      ...
      <GlassCard className="max-w-sm mx-auto mt-20">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="submit" className="flex-1 gradient-btn text-white py-3 rounded-xl font-medium">
              登录
            </button>
            <button type="button" onClick={handleRegister}
              className="flex-1 glass glass-hover text-gray-300 py-3 rounded-xl font-medium transition-colors">
              注册
            </button>
          </div>
        </form>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full gradient-btn flex items-center justify-center text-sm">
      ...
      <div className="grid gap-4">
        <AnimatePresence>
          {coupons.length === 0 ? (
            <GlassCard>
              <p className="text-gray-500 text-center py-8">暂无进行中的优惠券活动</p>
            </GlassCard>
          ) : (
            coupons.map(c => {
              const remaining = c.remainStock || 0
              ...
              return (
                <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <GlassCard className="glass-hover transition-all">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-bold">{c.couponName || '优惠券'}</h3>
                        <p className="text-xs text-gray-500 mt-1">
                          {isActive ? '⏰ 进行中' : '⏰ 已结束'} · 每人限{c.perUserMax || 1}张
                        </p>
                      </div>
                      <span className={`text-sm px-3 py-1 rounded-full text-xs font-medium ${
                        isActive ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-500'
                      }`}>
                        {isActive ? '进行中' : '已结束'}
                      </span>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          style={{
                            width: `${100 - pct}%`,
                            background: pct > 50 ? 'linear-gradient(90deg, #6366f1, #8b5cf6)'
                              : pct > 20 ? 'linear-gradient(90deg, #f59e0b, #ef4444)'
                    ...
                    <button
                      onClick={() => handleSeckill(c.id)}
                      disabled={!isActive || loading}
                      className={`w-full py-3 rounded-xl font-bold text-sm transition-all ${
                        isActive
                          ? 'gradient-btn text-white pulse-glow'
            ...
            <div className="glass rounded-2xl p-8 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
              <div className="text-center">
                ...
                <button onClick={() => setResult(null)}
                  className="gradient-btn text-white px-6 py-2 rounded-xl text-sm">
                  确定
                </button>
              </div>
            </div>
      ...
      {orders.length > 0 && (
        <GlassCard>
          <h3 className="text-sm font-semibold text-gray-400 mb-3">📋 我的订单</h3>
          <div className="space-y-2">
            {orders.slice(0, 5).map((o, i) => (
              <div key={i} className="flex justify-between items-center text-xs py-2 border-b border-white/5 last:border-0">
                <span className="font-mono text-gray-400">{o.orderNo?.slice(0, 12)}...</span>
                <span className={`px-2 py-0.5 rounded text-xs ${
                  o.status === 'CREATED' ? 'bg-blue-500/10 text-blue-400'
