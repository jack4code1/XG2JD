...
export default function AIPage() {
  ...
  const execute = async () => {
    ...
    try {
      ...
      setExecMsg(data.success ? `✅ 已创建活动: ${data.couponName} (ID:${data.couponId})` : `❌ ${data.message}`)
    } catch(e) { setExecMsg('❌ 执行失败') }
    setExecuting(false)
  }
        ...
        <div className="card-body">
          <div className="flex gap-3">
            <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit(e)}
              placeholder="描述你的需求..." className="input input-bordered flex-1" />
            <button onClick={submit} disabled={loading} className="btn btn-primary">
              {loading ? <span className="loading loading-spinner" /> : '🚀 策划'}
            </button>
          </div>
          <div className="flex gap-2 mt-2">
            {EXAMPLES.map((ex,i) => (
              <button key={i} onClick={() => { setQuery(ex); submit() }}
                className="btn btn-xs btn-outline">{ex}</button>
            ))}
          </div>
        </div>
      </div>
          ...
          <div className="card-body">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => navigator.clipboard.writeText(plan)} className="btn btn-xs btn-outline">📋 复制</button>
                <button onClick={execute} disabled={executing} className="btn btn-xs btn-primary">
                  {executing ? <span className="loading loading-spinner" /> : '🚀 一键创建'}
                </button>
              </div>
            </div>
            <pre className="whitespace-pre-wrap text-sm leading-relaxed opacity-80">{plan}</pre>
