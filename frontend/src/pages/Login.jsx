...
export default function Login() {
  ...
  const handleSubmit = async (e, isRegister) => {
    ...
    try {
      ...
      nav(role === 'MERCHANT' ? '/admin' : '/')
    } catch (e) {
      setMsg(e.response?.data?.message || '用户名或密码错误')
    }
  }
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          ...
          <div className="tabs tabs-boxed justify-center my-2">
            <a className={`tab ${role==='USER'?'tab-active':''}`} onClick={()=>setRole('USER')}>👤 用户</a>
            <a className={`tab ${role==='MERCHANT'?'tab-active':''}`} onClick={()=>setRole('MERCHANT')}>🏪 商家</a>
          </div>
          <form onSubmit={(e) => handleSubmit(e, true)}>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={(e) => handleSubmit(e, false)}
                className="btn btn-outline flex-1">登录</button>
            </div>
          </form>
